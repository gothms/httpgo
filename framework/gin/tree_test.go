...
func TestTreeWildcard(t *testing.T) {
	...
	checkRequests(t, tree, testRequests{
		...
		// fix PR: https://github.com/gothms/httpgo/framework/gin/pull/2796
...
func TestTreeWildcardConflictEx(t *testing.T) {
	...
	for _, conflict := range conflicts {
		...
		/*
			   ...
			   tree_test.go:971: [[/]]
			   tree_test.go:971: [[/] [who/ con]]
			   tree_test.go:971: [[/] [who/ con] [are foo/hello :tact]]
			   tree_test.go:971: [[/] [who/ con] [are foo/hello :tact] []]
			   tree_test.go:971: [[/] [who/ con] [are foo/hello :tact] [] [/*you]]
			   tree_test.go:973: [[/] [who/ con] [are foo/hello :tact] [] [/*you]]
		*/
		/*
				// 两个
				//routes = [...]string{
			   ...
			   tree_test.go:959: [[/who/]]
			   tree_test.go:959: [[/who/] [are foo/hello]]
			   tree_test.go:959: [[/who/] [are foo/hello] []]
			   tree_test.go:959: [[/who/] [are foo/hello] [] [/*you]]
			   tree_test.go:961: [[/who/] [are foo/hello] [] [/*you]]
				"/who/foo/hello": [[/who/are] [] [/*you]]
		*/
		for _, route := range routes {
			tree.addRoute(route, fakeHandler(route))
			//catchPanic(func() { tree.addRoute(route, fakeHandler(route)) })
			//printTree(tree)
		}
...
func printTree(tree *node) {
	queue := []*node{tree}
	paths, idx := make([][]string, 0), 0
	for l := len(queue); l > 0; l = len(queue) {
		paths = append(paths, []string{})
		for i := 0; i < l; i++ {
			curr := queue[i]
			paths[idx] = append(paths[idx], curr.path)
			//paths[idx] = append(paths[idx], curr.path+"#"+curr.fullPath)
